      ...
      for (; lidx < lmin_max; ++lidx) { // search through submodel with same lambda and alpha values
        ...
        for (int i = 0; i < cvModelBuilders.length; ++i) {  // run cv for each lambda value
          ...
          if (g._model._output._submodels[lidx] != null) {
            ...
            g._driver.computeSubmodel(lidx, lambda);
...
//        });
    ...
    protected Submodel computeSubmodel(int i,double lambda) {
      ...
      if(lambda >= _lmax && _state.l1pen() > 0)
        _model.addSubmodel(sm = new Submodel(lambda, _state.alpha(), getNullBeta(),_state._iter,_nullDevTrain, _nullDevValid));
      else {  // this is also the path for HGLM model
        sm = new Submodel(lambda, _state.alpha(), _state.beta(),_state._iter,_nullDevTrain, _nullDevValid);// restart from last run
        ...
        if (_parms._HGLM) {
          sm = new Submodel(lambda, _state.alpha(), _state.beta(), _state._iter,_nullDevTrain, _nullDevValid);
          sm.ubeta = Arrays.copyOf(_state.ubeta(),_state.ubeta().length);
          ...
          double xvalDev = ((_xval_deviances == null) || (_xval_deviances.length <= i)) ? Double.NaN : _xval_deviances[i];
          double xvalDevSE = ((_xval_sd == null) || (_xval_deviances.length <= i)) ? Double.NaN : _xval_sd[i];
    ...
    private transient double _nullDevValid = Double.NaN;
    ...
    public void computeImpl() {
      ...
      for (int alphaInd = 0; alphaInd < _parms._alpha.length; alphaInd++) {
        ...
        for (int i = 0; i < _parms._lambda.length; ++i) {  // for lambda search, can quit before it is done
          ...
          Submodel sm = computeSubmodel(submodelCount, _parms._lambda[i]);
...
/*      if (_vcov != null) {
    ...
    protected void updateProgress(boolean canScore){
      ...
      if(canScore && (_parms._score_each_iteration || timeSinceLastScoring() > _scoringInterval)) {
        _model.update(_state.expandBeta(_state.beta()), _nullDevTrain, _nullDevValid, _state._iter);
        scoreAndUpdateModel();
        _earlyStop = updateEarlyStop();
      }
    }
